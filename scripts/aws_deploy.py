#!/usr/bin/env python3
"""
aws_deploy.py — Pipeline de despliegue de Terraform en AWS (equivalente Python del workflow YAML).

Replica la lógica de .github/workflows/aws-terraform-deploy.yml en cinco etapas:
  ...
  Etapa 5 → Apply                (aprobación explícita + terraform apply)

Diferencias clave respecto a los scripts de Azure y GCP:
  - Autenticación: AWS usa las variables de entorno estándar del SDK de AWS
    (AWS_ACCESS_KEY_ID + AWS_SECRET_ACCESS_KEY + AWS_SESSION_TOKEN) o un
    perfil local (~/.aws/credentials). En GitHub Actions las credenciales
    las inyecta la action 'aws-actions/configure-aws-credentials@v4' via OIDC.
  - Backend: AWS usa S3 + DynamoDB con una clave por entorno
    (key=aws/<env>.terraform.tfstate). El lock table evita aplicaciones
    concurrentes que puedan corromper el estado.
  - Plugin de tflint: tflint-ruleset-aws (v0.31.0), configurado en aws/.tflint.hcl.
  - Región por defecto: us-east-1 (definida en AWS_REGION, sobrescribible via variable).
  - Entorno de GitHub: 'aws-dev' en el YAML; aquí se construye como 'aws-<env>'.

Uso:
  python scripts/aws_deploy.py                      # pipeline completo, env=dev
  python scripts/aws_deploy.py --env uat            # entorno UAT
  python scripts/aws_deploy.py --force-destroy yes  # permitir destrucciones
  python scripts/aws_deploy.py --plan-only          # solo hasta la etapa de plan
  python scripts/aws_deploy.py --skip-secrets-scan  # omitir gitleaks

Autenticación con AWS (una de las siguientes opciones antes de ejecutar):
  # Opción A — Perfil local (recomendada para uso local):
  aws configure   # crea ~/.aws/credentials con el perfil por defecto

  # Opción B — Variables de entorno con credenciales de acceso:
  export AWS_ACCESS_KEY_ID=AKIA...
  export AWS_SECRET_ACCESS_KEY=...
  export AWS_REGION=us-east-1

  # Opción C — Rol asumido con sesión temporal (CI / SSO):
...
def check_auth_env() -> None:
    """
    Verifica que haya credenciales AWS disponibles antes de intentar cualquier
    comando de Terraform que contacte la API de Amazon Web Services.

    AWS acepta credenciales de tres formas (en orden de precedencia):

      1. Variables de entorno AWS_ACCESS_KEY_ID + AWS_SECRET_ACCESS_KEY
         (opcionalmente AWS_SESSION_TOKEN para roles asumidos / SSO).
      2. Perfil local en ~/.aws/credentials (generado por 'aws configure').
      3. IAM Role adjunto a la instancia EC2 / contenedor ECS / pod EKS
         (credenciales del Instance Metadata Service — solo en entornos AWS).

    En GitHub Actions las credenciales las inyecta automáticamente la action
    'aws-actions/configure-aws-credentials@v4' via OIDC → rol IAM.
    Para ejecución local se recomienda la opción A (perfil) o B (variables).
    """
    # Opción 1: Variables de entorno explícitas.
    access_key = os.getenv("AWS_ACCESS_KEY_ID", "")
    secret_key  = os.getenv("AWS_SECRET_ACCESS_KEY", "")

    if access_key and secret_key:
        # Advertir si parece una clave de acceso permanente (AKIA...) en lugar
        # de una temporal (ASIA...) para fomentar el uso de roles/SSO.
        if access_key.startswith("AKIA"):
            warn(
                "Usando clave de acceso permanente (AKIA). "
                "Considera usar roles IAM o SSO para mayor seguridad."
            )
        else:
            info("Credenciales AWS: variables de entorno (credenciales temporales).")
        return

    # Opción 2: Perfil local ~/.aws/credentials.
    credentials_file = Path.home() / ".aws" / "credentials"
    config_file      = Path.home() / ".aws" / "config"

    if credentials_file.exists() or config_file.exists():
        # Verificar que 'aws sts get-caller-identity' funcione con el perfil activo.
        # Si falla, el perfil existe pero no tiene credenciales válidas.
        resultado = run(
        ...
        if resultado.returncode == 0:
            info("Credenciales AWS: perfil local (~/.aws/credentials).")
            return
        else:
            # El archivo existe pero las credenciales no son válidas o han expirado.
            error(
                "El archivo ~/.aws/credentials existe pero las credenciales no son válidas.\n"
                "  Ejecuta 'aws configure' o 'aws sso login' para renovarlas."
            )
            sys.exit(1)

    # No se encontró ninguna forma de autenticación.
    error(
        "No se encontraron credenciales AWS. Usa una de estas opciones:\n"
        "  A) aws configure                        (perfil local)\n"
        "  B) export AWS_ACCESS_KEY_ID=AKIA...     (variables de entorno)\n"
        "     export AWS_SECRET_ACCESS_KEY=...\n"
        "  C) aws sso login --profile <nombre>     (SSO empresarial)"
    )
    sys.exit(1)
...
def validate(working_dir: Path, env_name: str) -> None:
    """
    Ejecuta la suite completa de validación sobre el código Terraform de AWS.

    Pasos (replica el job 'validate' del YAML):
      2a. terraform fmt -check -recursive   — verificación de formato (sin escritura)
      2b. terraform init                    — inicializar backend S3 y proveedores
      2c. terraform validate                — verificación de sintaxis y tipos
      2d. tflint --init && tflint --recursive — linting con plugin tflint-ruleset-aws
      2e. checkov                           — escaneo de seguridad y cumplimiento AWS

    El backend de AWS usa una clave S3 (key=aws/<env>.terraform.tfstate) que separa
    el estado por entorno. El DynamoDB lock table 'jproject-tfstate-lock' evita
    aplicaciones concurrentes que puedan corromper el estado.
    """
    ...
    # ── 2b. Terraform init ────────────────────────────────────────────────────
    # La clave del backend S3 sigue el patrón aws/<env>.terraform.tfstate para
    # que cada entorno tenga su propio objeto de estado en el bucket compartido.
    info("Inicializando backend de Terraform (S3 + DynamoDB) ...")
    backend_key = f"aws/{env_name}.terraform.tfstate"
    run(
        ["terraform", "init", "-reconfigure", f"-backend-config=key={backend_key}"],
        cwd=working_dir,
    )
    ...
    # ── 2d. tflint ────────────────────────────────────────────────────────────
    # AWS usa el plugin tflint-ruleset-aws (v0.31.0), configurado en
    # aws/.tflint.hcl. --init descarga el plugin en el primer uso.
    info("Ejecutando tflint (plugin aws) ...")
    ...
    # Segunda pasada: salida compacta legible — informativa, no bloquea el pipeline.
    run(
...
# Solo esta persona puede autorizar cambios en infraestructura AWS.
...
def request_approval(env_name: str) -> None:
    ...
    Equivale al 'required reviewer' del GitHub Environment 'aws-dev' en el YAML.
    ...
    print()
    warn("Esta accion desplegara o modificara infraestructura real en AWS.")
...
def apply(working_dir: Path, plan_file: Path, env_name: str) -> None:
    """
    Solicita aprobación y aplica el archivo de plan generado previamente en AWS.

    El pipeline YAML protege este paso con el GitHub Environment 'aws-dev',
    ...
    success("terraform apply completado — infraestructura AWS actualizada.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Pipeline de despliegue de Terraform en AWS (equivalente Python del workflow YAML).",
...
def main() -> None:
    ...
    # Localizar la raíz del repositorio y el directorio de trabajo aws/.
    # __file__ apunta a scripts/aws_deploy.py -> parent.parent es la raíz.
    repo_root   = Path(__file__).resolve().parent.parent
    working_dir = repo_root / "aws"

    if not working_dir.is_dir():
        error(f"Directorio aws/ no encontrado en {working_dir}")
        sys.exit(1)

    # Cabecera del pipeline.
    print(_color(f"\n{'=' * 60}", BOLD))
    print(_color(f"  Pipeline AWS Terraform  |  env={args.env}", BOLD))
    print(_color(f"{'=' * 60}\n", BOLD))

    # Verificar credenciales AWS antes de hacer cualquier trabajo.
    check_auth_env()
    ...
    print()
    success("Pipeline AWS finalizado exitosamente.")
